...
public class SftpEnrico {
	public static void main(String[] arg) {

		try {
			JSch jsch = new JSch();

			String host = "kissako.it";
			String user = "root";// host.substring(0, host.indexOf('@'));
			host = "kissako.it";//host.substring(host.indexOf('@')+1);
			int port = 7822;
	...
	public static class MyUserInfo implements UserInfo, UIKeyboardInteractive {
		...
		public boolean promptYesNo(String str) {
			...
			int foo = JOptionPane.showOptionDialog(null, str, "Warning", JOptionPane.DEFAULT_OPTION, JOptionPane.WARNING_MESSAGE, null,
					options, options[0]);
			return foo == 0;
		}
	...
	/*public static class MyProgressMonitor implements com.jcraft.jsch.ProgressMonitor{ JProgressBar progressBar; JFrame frame; long
	 * count=0; long max=0;
	 * 
	 * public void init(String info, long max){ this.max=max; if(frame==null){ frame=new JFrame(); frame.setSize(200, 20); progressBar = new
	 * JProgressBar(); } count=0;
	 * 
	 * frame.setTitle(info); progressBar.setMaximum((int)max); progressBar.setMinimum((int)0); progressBar.setValue((int)count);
	 * progressBar.setStringPainted(true);
	 * 
	 * JPanel p=new JPanel(); p.add(progressBar); frame.getContentPane().add(progressBar); frame.setVisible(true);
	 * System.out.println("!info:"+info+", max="+max+" "+progressBar); } public void count(long count){ this.count+=count;
	 * System.out.println("count: "+count); progressBar.setValue((int)this.count); } public void end(){ System.out.println("end");
